	...
	static class Program
	{
        ...
        internal static void Launch(string[] args)
        {
            if (args.Length > 0)
            {
                ...
                {
                    proxy.NotifyOperator(inputInfo.Name, "UNSUPPORTED FILE TYPE", String.Format("The above file, found under {0}, is not a managed Assembly file (DLL or EXE) or targets a .NET runtime newer than 4.0.", inputInfo.DirectoryName), "Try again with a valid managed Assembly file or upgrade to a newer version of SmokeTest.", MessageBoxIcon.Exclamation);
                }
